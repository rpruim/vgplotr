...
for (name in sort(names(mark_defs))) {
  ...
  mark_lines <- c(mark_lines, generate_wrapper(
    ...
    extra_docs = c(
      "#' @param data_from The name of the data source this mark reads from (see [vg_data()]); a length-1 R integer (`1L`, `-1L`, ...; note the `L`) giving a 1-based index into the spec's registered data sources instead, negative counting from the end (e.g. `-1L` for the most recently registered one); or a literal vector of values to use as inline data directly (mosaic-spec's `\"data\": [...]` shorthand, e.g. for a single reference line -- a bare, non-integer number like `0` or `c(0)` means this, not an index). Left unset, defaults to the first registered data source (equivalent to `data_from = 1L`) for any mark type that takes data at all.",
